...
$('.btn-itemProperty').on('click', function() {
    activeStatus = !$("option:selected", this).hasClass('choose');
    console.log(activeStatus);
    if(!$("option:selected", this).hasClass('choose')){
        attrType = $("option:selected", this).attr('data-attr-type');
        attrValueId = $("option:selected", this).attr('data-attr-value-id');
    }
});

$('.btn-itemProperty').on('change', function() {
    var ItemType = $("option:selected", this).data('type');
    activeStatus = !$("option:selected", this).hasClass('choose');
    //console.log(activeStatus);
    if(ItemType !== '' /*&& !$(this).hasClass('disabled')*/) {
        if($("option:selected", this).hasClass('choose')) {
            //$(this).removeClass('active')
            delete product_arrayTemp_click['key' + $("option:selected", this).attr('data-attr-type')];
            $('#p_a_w' + $('option:selected', this).attr('data-attr-type')).attr('data-sel', 0);
            //if( $('#productsku').val()) {
              //  $('#productsku').val('');
            //}
            console.log(1111);
           ...
           // $('.btn-itemProperty[data-type=' + ItemType + ']').removeClass('active');
            ...
            $('#p_a_w' + $('option:selected', this).attr('data-attr-type')).attr('data-sel', 1);
    ...
    var nowClickArray = product_getNowClickArray(attrType, attrValueId, activeStatus);
    product_onclickStatic(nowClickArray, attrType, activeStatus);
