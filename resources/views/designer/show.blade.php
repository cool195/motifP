@include('header',['title'=>'MOTIF | '.$designer['nickname'],'description'=>$designer['describe'],'ogimage'=>config('runtime.CDN_URL').'/n0/'.$designer['img_video_path']])
                    ...
                    @if($value['type']=='banner' || (!isset($value['spus']) && $value['type']=='product'))
                        <!--banner图-->
                            <div class="m-t-20x text-center">
                                ...
                                @else
                                    <a href="@if($value['skipType']=='1')/detail/{{$value['skipId']}}{{$value['skipId']}}@elseif($value['skipType']=='2')/designer/{{$value['skipId']}}@elseif($value['skipType']=='3')/topic/{{$value['skipId']}}@elseif($value['skipType']=='4')/shopping#{{$value['skipId']}}@else{{$value['skipId']}}@endif"
